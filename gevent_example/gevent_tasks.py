...
start celery with one of command above.
start the script to distribute task and fetch result with:
    python gevent_tasks.py

the script to distribute task and fetch result could be moved to another file.
"""
...
# celery = Celery('gevent_tasks', broker='redis://localhost:6379/0',
#                 backend='redis://localhost/0')
# read config from config file
celery = Celery('gevent_tasks')
...
if __name__ == "__main__":
    ...
    # map(get_res, url_l)
    # get_res(test_url)  # different tasks
    get_cal_res(100)
